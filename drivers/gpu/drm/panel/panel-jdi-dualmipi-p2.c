...
{
	return container_of_const(panel, struct jdi_dualmipi0, panel);
}
...
{
	return drm_connector_helper_get_modes_fixed(connector, &jdi_dualmipi0_mode);
}
...
static int jdi_dualmipi0_probe(struct mipi_dsi_device *dsi)
{
	...
	ctx = devm_drm_panel_alloc(dev, struct jdi_dualmipi0, panel,
				   &jdi_dualmipi0_panel_funcs,
